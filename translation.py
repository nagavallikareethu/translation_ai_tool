...
def verify_coordinate_preservation(original_data, translated_data):
    ...
    def compare_bbox_sequences(orig_seq, trans_seq, label, page_idx):
        ...
        if len(orig_seq) != len(trans_seq):
            print(f"❌ Page {page_idx + 1}: {label} count mismatch ({len(orig_seq)} vs {len(trans_seq)})")
        ...
        for idx, (orig_item, trans_item) in enumerate(zip(orig_seq, trans_seq), start=1):
            ...
            if orig_bbox != trans_bbox:
                print(f"❌ Page {page_idx + 1}, {label} {idx}: coordinates changed!")
...
def run_full_pipeline(pdf_path: str,
                      ...
                      overlay: bool = True):
    """Run extraction → translation → PDF creation workflow."""
    ...
    for lang_code in languages:
        ...
        if not translated_path:
            print(f"⚠️  Translation skipped for {lang_name}")
            continue
        ...
        if overlay:
            ...
            try:
                ...
                print(f"❌ PDF generation failed for {lang_name}: {pdf_error}")
    
    print("\n=== PIPELINE SUMMARY ===")
    print(f"📄 Source PDF        : {pdf_file}")
    print(f"🧾 Extracted JSON    : {extracted_path}")
    print(f"🌐 Languages         : {', '.join(languages)}")
    print(f"💾 Translated JSONs  : {len(translated_results)} files in {translated_dir_path.resolve()}")
    if overlay:
        print(f"📚 Generated PDFs    : {len(generated_pdfs)} files in {output_dir_path.resolve()}")
    else:
